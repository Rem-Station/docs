# Rem Station Banning Policy

```admonish info
This is the ban policy for Rem Station's servers. It applies only to our servers.
```

```admonish info
This page is mostly taken from Wizden's Banning Policy, and using Paradise Station's way of handling bans appeals.
```
...
Looking for the [Admin Policy and Conduct](https://docs.remstations.com/en/community/admin/wizards-den-admin-policy.html?) page?
...
### Voucher Bans
Issued due to continued poor behavior, extreme griefing, ban-evasion or otherwise as a direct result of player actions. Unless the ban is found to be outrageously unwarranted by staff, these bans will require either a two or six month waiting period and a vouch for good behavior from another server before the ban will be lifted. Only Head Game Administrators may handle appeals involving “permanent until appeal with vouch” bans, and they are not subject to individual admin discretion to be lifted early.

#### Soft Voucher Bans
Soft Voucher Bans are issued from continued poor behavior. Soft Voucher Bans are only appealable after a two month waiting period.

#### Hard Voucher Bans
Hard Voucher Bans are issued either from ban evasion, or continued poor behavior after a Soft Voucher Ban. Hard Voucher Bans are only appealable after a six month waiting period.
...
Alternate account bans should typically be in the forms
`Alt of OriginalUsername` 
**OR**
`AltUsername alt of OriginalUsername`.
...
Self-antag and station sabotage are part of the same grouping category, so they are treated as only station sabotage. Both remaining offenses can have the new player modifier applied if the player is new to the game, which would allow the offenses to result in only a warning and an instruction to read the rules. The station sabotage offense may optionally have the role-specific modifier applied. Any of the following total suggestions are valid:
...
# Appeals
Appeals are handled by the banning admin, or another admin if the banning admin cannot handle the appeal.

Voucher Bans are to be made known of to the Head Game Administrators before being placed.
...
- The processor has somehow indicated that they are not going to process the appeal.
**OR**
- A Head Game Administrator has told you that you can process the appeal.

## Processing by Trial Game Administrators
Trial Game Administrators may not process appeals without the supervision and approval of a Full Game Administrator. Before an outcome can be made it must be approved by the supervising Game Administrator.

Any responses should be approven beforehand by the supervising Game Administrator.
...
3. If you are not the banning admin and the appeal is extremely low effort, or a "troll appeal" deny the appeal. An appeal is considered to be low effort if it meets any of the following critera:
    ...
    - The appealing player is clearly not expecting their appeal to be accepted, and made it simply to troll. 
...
4. If you are not the banning admin, you may not process the appeal unless the banning admin is not able to handle it themself, or if the banning admin gives you permission to handle the ban.
5. Collect information
	...
	5. If you are handling the ban in place of the banning admin, contact them for any needed information.
