...
2. Download the latest version of the server from [our builds page](https://cdn.remstations.com/fork/RemStation) for your operating system. If you are looking for another fork, ask that fork if they have a server builds page. Otherwise refer to the [Custom Code](#level-2-server-with-custom-code) section below.
...
If you are having trouble understanding what to click, here is a quick video. Subtitles contain some extra information if needed. The video is mainly for hosting a Wizard's Den server, but you should be able to follow along with starting a Rem Station server as normal.
...
* You can use `promotehost` command from the server console (e.g. `promotehost Player`) to temporarily give a connected client host.
...
Support for MySQL/MariaDB isn't currently planned, but the offcial Space Wizards Space Station 14 repository accepts contributions.
...
* [Official Builds](https://cdn.remstations.com/fork/RemStation)
* [Wizard's Den Infrastructure Reference](https://docs.spacestation14.com/en/community/infrastructure-reference/wizards-den-infrastructure.html)
* [Space Station 14 Public Hub Server Rules](https://docs.spacestation14.com/en/community/space-wizards-hub-rules.html)
